...
it("teacher sends only the entered email and selected class, then shows delivery status", async () => {
  ...
  expect(screen.getByRole("button",{name:"Invitation table"}).getAttribute("aria-expanded")).toBe("false");
  expect(screen.getByRole("textbox",{name:"Paste a student list"})).toBeTruthy();
  expect(screen.getByRole("button",{name:"Send invitation"})).toBeTruthy();
  expect(screen.queryByRole("table")).toBeNull();
  fireEvent.click(screen.getByRole("button",{name:"Invitation table"}));
